...
/*List of processess in the blocked state*/
...
static bool before(const struct list_elem *a, const struct list_elem *b, void *aux UNUSED){
  ...
  ta = list_entry(a, struct thread, elem);
  tb = list_entry(b, struct thread, elem);
  return ta->waitTime<tb->waitTime;
}
...
/*Priority Functions -T01-----------------------------*/
void thread_priority_temporarily_up(){

  struct thread *cur = running_thread ();
  ASSERT(cur->prev_priority == -1);
	
  cur->prev_priority = cur->priority;
  cur->priority = PRI_MAX;
 
}

void thread_priority_restore(int64_t ticks){
  struct list_elem *e;
  for (e = list_begin (&sleepers); e != list_end (&sleepers);
       e = list_next (e))
    {
      ...
      if(t->waitTime <= ticks){
        ASSERT(t->prev_priority!=-1 && t->priority==PRI_MIN);
...
void thread_block_till(int64_t waitTick){
 ...
 // lock_init((struct lock *)t);
  //ASSERT(lock_try_acquire((struct lock *)t));
  t->waitTime = waitTick;

  list_insert_ordered(&sleepers, &t->waitElem, (list_less_func*)&before, NULL);
